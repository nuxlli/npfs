...
static int rdma_trans_read(u8 *data, u32 count, void *a);
static int rdma_trans_write(u8 *data, u32 count, void *a);
...
np_rdmatrans_create(struct rdma_cm_id *cmid, int q_depth, int msize)
{
	...
	trans = np_trans_create(rdma, rdma_trans_read, rdma_trans_write,
		rdma_trans_destroy);
	if (!trans)
		goto error;
...
again:
	if (rdma->rfirst) {
		...
		memmove(data, ctx->buf + ctx->pos, n);
		ctx->pos += n;
		if (ctx->pos == ctx->len) {
...
static int
rdma_trans_write(u8 *data, u32 count, void *a)
{
	int i, n;
	Rdmatrans *rdma;
	struct ibv_sge sge;
